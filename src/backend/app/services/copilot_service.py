...
def _rules_fallback(ctx: CopilotContextSnapshot, question: str) -> str:
    ...
    lines = [
        ...
        f"⚠️ Synthetic demo data only.",
        "",
        f"**Current situation:**",
        f"- Peak congestion risk: {risk} ({risk_pct:.0f}%)",
        f"- Active vessels: {ctx.active_vessel_count} "
        f"({ctx.arrivals_next_24h} arriving in next 24 h)",
        f"- Average estimated wait: "
        f"{ctx.avg_estimated_waiting_minutes:.0f} min "
        f"({ctx.avg_estimated_waiting_minutes/60:.1f} h)",
    ]
    ...
    lines += [
        "",
        "**3 operational recommendations (from structured context):**",
    ]
    ...
    lines += [
        "",
        "_This explanation was generated by rules_fallback from structured context data. "
        "No LLM was used. All data is synthetic and not validated for real-world operations._",
    ]
...
def _ibm_bob_ask(
    ...
    req = urllib.request.Request(
        f"{base_url.rstrip('/')}/ml/v1/text/generation?version=2024-01-01",
        data=payload,
        headers={
